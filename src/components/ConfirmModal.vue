...
<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="cp-modal relative mx-3 w-full max-w-md">
      <header class="mb-3 flex items-center justify-between">
        <h3 class="cp-title">{{ title || 'Confirmation' }}</h3>
        <button class="cp-panel px-2 py-1" @click="emit('cancel')">Fermer</button>
      </header>
      <p class="mb-4 text-sm opacity-90">{{ message || 'Êtes-vous sûr ?' }}</p>
