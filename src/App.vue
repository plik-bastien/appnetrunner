...
onMounted(() => {
  isSwapped.value = localStorage.getItem('nr-swap') === '1'
  showIcons.value = localStorage.getItem('nr-icons') === '1'
})

watch(showIcons, (v) => {
  localStorage.setItem('nr-icons', v ? '1' : '0')
})

function toggleSwap() {
  isSwapped.value = !isSwapped.value
  localStorage.setItem('nr-swap', isSwapped.value ? '1' : '0')
...
<template>
  ...
  <main class="main-screen relative" :class="{ 'is-swapped': isSwapped }">
        ...
        <div class="section-inner">
          <div class="rotor-wrap">
            <div class="rotor rotor--runner">
              <div class="side-wrap">
                <SideBoard side="runner" :show-icons="showIcons" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="section-inner">
          <div class="rotor-wrap">
            <div class="rotor rotor--corp">
              <div class="side-wrap">
                <SideBoard side="corp" :show-icons="showIcons" />
              </div>
            </div>
          </div>
        </div>
    ...
    <!-- ancien confirm -->
    <ConfirmModal :open="askReset" title="Réinitialiser la partie"
      message="Ceci remettra tous les compteurs à zéro pour les deux camps." @confirm="doReset"
    ...
    <div v-if="menuOpen" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="cp-panel relative mx-4 w-full max-w-sm p-4 z-10">
        ...
        <div class="space-y-2">
          <!-- 1) SWITCH ICONES / INTITULÉS (EN PREMIER) -->
          <label class="ui-switch cp-panel w-full px-3 py-3 cursor-pointer select-none">
            <input type="checkbox" v-model="showIcons" />
            <span class="track"><span class="knob"></span></span>
            <span class="label-text">Afficher les icônes</span>
          </label>
    ...
    <!-- confirm reset (menu) -->
    <ConfirmModal :open="askReset || confirmReset" title="Réinitialiser la partie"
      message="Ceci remettra tous les compteurs à zéro pour les deux camps." @confirm="doReset(); confirmReset = false"
